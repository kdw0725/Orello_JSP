...
public class ApointOk extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		ArrayList<ChartDTO> list = dao.memberlist(map);
		...
		if (n == 1) {
			...
			pagebar += "<li>";
			pagebar += String.format("<a href=\"/orello/achart/apointok.do?page=%d&startdate=%s&enddate=%s\" aria-label=\"Previous\">",
					n - 1, start, finish);
			pagebar += "<span aria-hidden=\"true\">&laquo;</span>";
			pagebar += "</a>";
			pagebar += "</li>";
		...
		while (!(loop > blockSize || n > totalPage)) {
			...
			if (n == nowPage) {
				...
				pagebar += "<li>";
				pagebar += String.format("<a href=\"/orello/achart/apointok.do?page=%d&startdate=%s&enddate=%s\">%d</a>", n, start, finish, n);
				pagebar += "</li>";
			}
		...
		if (n > totalPage) {
			...
			pagebar += "<li>";
			pagebar += String.format("<a href=\"/orello/achart/apointok.do?page=%d\" aria-label=\"Next\">", n);
			pagebar += "<span aria-hidden=\"true\">&raquo;</span>";
			pagebar += "</a>";
			pagebar += "</li>";
		}
		...
		/*
		 * String page = req.getParameter("page"); String search =
		 * req.getParameter("searchtxt");
		 * 
		 * ArrayList<ChartDTO> list = dao.pointlist(map);
		 * 
		 * System.out.println(list); req.setAttribute("list", list);
		 */
